...
export const createFile = async (
  ...
  if (!response.ok) throw new Error("File creation request failed");
  return response.json();
};
...
export const updateLearning = async () => {
  ...
  if (!response.ok) throw new Error("Learning update request failed");
  return response.json();
};
...
export const analyzeFiles = async (files: File[], question: string) => {
  ...
  if (!response.ok) throw new Error("File QA request failed");
