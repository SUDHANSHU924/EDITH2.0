...
export function useEdith(sessionId: string = "default", department: string = "core") {
  ...
  // EDITH greets the user on first launch — Jarvis-style boot greeting
  const greet = useCallback(async () => {
    ...
    try {
      const stream = await fetch("/api/edith/greet");
      if (!stream.ok) throw new Error("greet unavailable");
      await streamEdithResponse(stream, {
        onToken: (token) => appendToMessage(id, token, "core"),
        onDone: () => updateMessage(id, { isStreaming: false }, "core"),
        onError: () =>
          updateMessage(
            id,
            { content: "Good day, sir. All 15 modules are online and standing by.", isStreaming: false },
            "core"
          ),
      });
    } catch {
      updateMessage(
        id,
        { content: "Good day, sir. EDITH is online. What are we working on today?", isStreaming: false },
        "core"
      );
    }
  ...
  return { sendMessage, isThinking, greet };
}
